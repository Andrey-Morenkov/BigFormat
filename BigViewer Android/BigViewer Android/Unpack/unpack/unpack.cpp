...
//  2:1   1:1   1:2   1:4   1:8
//  8     7     6     5     4
int decodeX (uint8 * src, uint8 * dst,	short int len, int lnSize)
{
  const int rSize = (1 << lnSize); // row size (std - 128)
  const int olnSize = (lnSize == 8) ? 8 : 7; // output row size (128/256)
  const int bSize = rSize + 1; // fbtr row size (std - 129)
  const int sStep = (1 << (14 - lnSize)); // line step in 128*128 src
  const int lnZoom = 7 - lnSize; // ln shift zoom factor

  uint8 oc = 0xFF;
  if (
      (len == 0) ||
      ((len == 1) && (src[0] == 0xFF)) ||
			((len == 1) && (src[0] == 0) && (oc = 0, true)) || // set black
      ((len == 2) && (src[0] == 1) && (oc = src[1], true)) // set any color
     )
  {
    // One color
    if (lnSize == 7)
  ...
  else if ((len > 1) && (len < 16384) && (src[0] == 0xFF))
  {
    ...
    switch (src[1])
    {
      ...
      case 3:
        ...
        break;
      case 4:
        Block = new TUniIndexedBtrBlock(&Coder, lnSize, false);
        break;
      default:
		  {
			delete[]pbuf;
  ...
  else
  {
    ...
    {
      static uint8 clbuf[16384]; // static for one-thread ONLY!
...
{
  return decodeX((uint8 *)source, (uint8 *)dest, len,7);
}

int decode_x0(void * source, void * dest, short int len)
{
  return decodeX((uint8 *)source, (uint8 *)dest, len,8);
}
...
{
  return decodeX((uint8 *)source, (uint8 *)dest, len,6);
}

int decode_x4(void * source, void * dest, short int len)
{
  return decodeX((uint8 *)source, (uint8 *)dest, len,5);
}

int decode_x8(void * source, void * dest, short int len)
{
  return decodeX((uint8 *)source, (uint8 *)dest, len,4);
}